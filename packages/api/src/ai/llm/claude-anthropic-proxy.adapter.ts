...
export class ClaudeAnthropicProxyAdapter extends BaseLLMAdapter {
  ...
  async *chatStream(input: LLMInput): AsyncGenerator<string, StreamResult, unknown> {
    ...
    const body: Record<string, unknown> = {
      ...
      stream: true,
      messages,
      // Раньше слали `tools: []` + `tool_choice: { type: "none" }` для
      // подавления KIE-инжекта built-in skills (модель дёргала `view` на
      // skill-файл вместо ответа). KIE починили инжект на своей стороне
      // (2026-05) — поля убраны. Регресс ловит tool_use-детектор ниже.
