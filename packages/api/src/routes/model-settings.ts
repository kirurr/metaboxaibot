...
export const modelSettingsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{ Params: { dialogId: string } }>("/model-settings/dialog/:dialogId", {
    schema: {
      description: "Get dialog-level model settings",
      params: { type: "object", properties: { dialogId: { type: "string", description: "Dialog ID" } }, required: ["dialogId"] },
      response: { 200: { type: "object" } },
    },
  }, async (request) => {
      const { userId } = request as AuthRequest;
  ...
  fastify.patch<{ Params: { dialogId: string }; Body: { settings: Record<string, unknown> } }>(
    ...
    schema: {
      ...
      params: { type: "object", properties: { dialogId: { type: "string", description: "Dialog ID" } }, required: ["dialogId"] },
      body: { type: "object", properties: { settings: { type: "object", description: "Settings object" } }, required: ["settings"] },
      response: { 200: { type: "object", properties: { success: { type: "boolean" } } }, 400: badRequestResponse },
    },
    }, async (request) => {
      const { userId } = request as AuthRequest;
      const { dialogId } = request.params;
