...
import { constructOpenAPIonRouteHook, badRequestResponse, userIsBlockedResponse } from "../utils/openapi.js";
...
export const authRoutes: FastifyPluginAsync = async (fastify) => {
   ...
   */
  fastify.post<{ Body: { initData: string } }>("/auth/verify", {
    schema: {
      ...
      response: {
        ...
        401: { type: "object", description: "Invalid initData hash or timing validation failed", properties: { error: { type: "string" } } },
        403: userIsBlockedResponse,
        404: { type: "object", description: "User not found — open the bot first", properties: { error: { type: "string" }, code: { type: "string", enum: ["USER_NOT_FOUND"] } } },
      },
    },
  }, async (request, reply) => {
   ...
   */
  fastify.post<{ Body: { token: string } }>("/auth/webtoken", {
    schema: {
      ...
      response: {
        ...
        400: badRequestResponse,
        401: { type: "object", description: "Invalid or expired token", properties: { error: { type: "string" } } },
        403: userIsBlockedResponse,
        404: { type: "object", description: "User not found — open the bot first", properties: { error: { type: "string" }, code: { type: "string", enum: ["USER_NOT_FOUND"] } } },
      },
    },
