 ...
 */
export function translateMentionsToCanonical(prompt: string, mentions: ActiveMention[]): string {
  let result = prompt;
...
export function buildElementMediaInputs(
  ...
  for (const { element, slotIndex } of mentions) {
    ...
    const keys =
      picked.length > 0
        ? picked.slice(0, maxImages)
        : element.media.slice(0, maxImages).map((mm) => mm.s3Key);
